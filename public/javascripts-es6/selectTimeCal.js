...
calTime.init = function (json) {
    let isToday = true, date = new Date(), hourUnit = "时", minUnit = "分", secUnit = "秒", type = "2", timeType = "2";
    if (json) {
        if (json.today) {
            isToday = json.today;
        }
    ...
    let minhour = 0, minmin = 0,minsec = 0;
    let maxhour = 24, maxmin = 60, maxsec = 60;
    let s = "[";
    if (isToday) {
        if ("2" === timeType) {
...
calTime.getSixtyOnly = function (json) {
    ...
    let s = "";
    for (let iii = min1; iii < max1; iii++) { //sec
        let iiistr = iii;
        if (iii < 10) {
            iiistr = "0" + iii;
        }
...
calTime.getSixtyAndOneChild = function (json) {
    let Unit1 = "分", Unit2 = "秒", min1 = 0, max1 = 60, min2 = 0, max2 = 60, timeType = "2";
    if (json) {
        if (json.Unit1)
            Unit1 = json.Unit1;
        if (json.Unit2)
    ...
    let getSixtyOnly = calTime.getSixtyOnly({
        Unit1: Unit2,
        timeType: timeType
    });
    let getSixtyOnly1 = calTime.getSixtyOnly({
        Unit1: Unit2,
        min1: min2,
        max1: max2,
        timeType: timeType
...
calTime.getSixtyAndTwoChild = function (json) {
    let Unit1 = "时", Unit2 = "分", Unit3 = "秒", min1 = 0, max1 = 24, min2 = 0, max2 = 60, min3 = 0, max3 = 60,
        timeType = "2";
    if (json) {
        if (json.Unit1)
            Unit1 = json.Unit1;
    ...
    let getSixtyAndOneChild = calTime.getSixtyAndOneChild({
        Unit1: Unit2,
        Unit2: Unit3,
        timeType: timeType
    });
    let getSixtyAndOneChild1 = calTime.getSixtyAndOneChild({
        Unit1: Unit2,
        Unit2: Unit3,
        min1: min2,
        max1: max2,
